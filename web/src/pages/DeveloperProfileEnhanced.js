          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-foreground">
                  {tByEn('Progress to')} {data.next_level}
                </p>
                <p className="text-sm font-bold text-signal">
                  {lang === 'uk' ? `потрібно ${pointsToNext} балів` : `${pointsToNext} ${tByEn('points needed')}`}
                </p>
              </div>
            ...
            <div className="p-4 bg-signal/10 rounded-lg border border-signal/30">
              <p className="font-semibold text-signal mb-2 flex items-center gap-2">
                ...
                {lang === 'uk' ? `Рівень ${data.next_level} відкриває:` : `${data.next_level} Level Unlocks:`}
              </p>
              <ul className="space-y-1 text-sm text-muted-foreground">
                <li className="flex items-center gap-2">
                  <DollarSign className="w-4 h-4" />
                  {tByEn('+10% earnings multiplier')}
                </li>
                <li className="flex items-center gap-2">
                  <Lock className="w-4 h-4" />
                  {tByEn('Access to $1500+ premium modules')}
                </li>
                <li className="flex items-center gap-2">
            ...
            <CardContent className="space-y-4">
              ...
              {data.strikes > 0 && (
                <div className="p-3 bg-[color:var(--danger-surface)] border border-[color:var(--danger-border)] rounded-lg" role="alert">
                  <p className="font-semibold mb-2 text-[color:var(--danger)]">⚠️ {tByEn('WARNING')}</p>
                  ...
                  <ul className="text-sm mt-2 space-y-1 ml-4 text-foreground">
                    <li>→ {data.strikes >= 2 ? '48h' : '24h'} {tByEn('marketplace block')}</li>
                    <li>→ {tByEn('Lose access to premium modules')}</li>
                    <li>→ {tByEn('Rating penalty (-5 points)')}</li>
                  </ul>
                </div>
              )}
              ...
              {data.strikes === 0 && (
                <div className="p-3 bg-[color:var(--success-surface)] border border-[color:var(--success-border)] rounded-lg">
                  <p className="text-sm text-foreground">
                    ...
                    {tByEn('Clean record! Keep delivering quality work.')}
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <Card className="border-2 border-[color:var(--warning-border)] bg-[color:var(--warning-surface)] shadow-[var(--shadow-elev-1)]" data-testid="growth-panel">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-[color:var(--warning)]">
                ...
                {tByEn('NEXT LEVEL')}: +${data.growth[0]?.monthly_impact || 920}/{tByEn('month')}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {tByEn('Current')}: {opp.current} → {tByEn('Target')}: {opp.target}
                        </p>
                      </div>
        ...
        <Card className="border border-border bg-card" data-testid="security-section">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {tByEn('Security · Two-factor authentication')}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between gap-4 p-4 rounded-lg border border-border bg-muted">
              <div>
                ...
                <p className="text-lg font-bold text-foreground mt-0.5">{tByEn('2FA management')}</p>
