      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl md:text-3xl font-bold text-foreground flex items-center gap-2 md:gap-3">
              ...
              <span className="truncate">{tByEn('Marketplace')}</span>
            </h1>
            <p className="text-xs md:text-sm text-muted-foreground mt-1">{tByEn('Take modules and start earning')}</p>
          </div>

          {/* Capacity Indicator — compact */}
          <div className="px-3 py-2 md:px-6 md:py-4 rounded-lg md:rounded-xl bg-card border border-border shrink-0" data-testid="capacity-indicator">
            <p className="text-[9px] md:text-sm text-muted-foreground mb-0.5 md:mb-1 uppercase tracking-wide leading-tight">{tByEn('Active Capacity')}</p>
            <p className={`text-lg md:text-3xl font-bold font-mono leading-none ${isFull ? 'text-[color:var(--danger)]' : 'text-[color:var(--success)]'}`}>
        ...
        <div>
          <h2 className="text-xl font-bold text-foreground mb-4">
            {tByEn('Available Work')} ({modules.length})
          </h2>
