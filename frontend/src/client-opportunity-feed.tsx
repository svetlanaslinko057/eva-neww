        ...
        return (
          <View key={o.opportunity_id} style={[s.card, o.priority === 'high' && s.cardHigh]} testID={`opp-${o.opportunity_id}`}>
            ...
            <View style={s.ctaRow}>
              ...
              >
                {acceptBusy
                  ? <><ActivityIndicator color={T.bg} size="small" /><Text style={s.ctaPrimaryText}>{'Adding…'}</Text></>
                  : <>
                      <Ionicons name="add-circle" size={14} color={T.bg} />
                      <Text style={s.ctaPrimaryText}>Add now · ${o.price}</Text>
                    </>}
              </TouchableOpacity>
              <TouchableOpacity
...
const s = StyleSheet.create({
  ...
  ctaRow: { flexDirection: 'column', gap: 8, alignItems: 'stretch' },
  ctaPrimary: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: T.primary, paddingVertical: 11, paddingHorizontal: 12, borderRadius: T.radiusSm },
  ctaPrimaryText: { color: T.bg, fontWeight: '800', fontSize: T.small, flexShrink: 1, textAlign: 'center' },
  ctaGhost: { alignItems: 'center', justifyContent: 'center', backgroundColor: T.surface2, borderWidth: 1, borderColor: T.border, paddingVertical: 11, paddingHorizontal: 12, borderRadius: T.radiusSm },
