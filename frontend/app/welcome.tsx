  ...
  return (
    ...
    >
      ...
      <View style={s.brand} testID="welcome-brand">
        ...
        />
        {/* Compact single-button language toggle. Auto-detected from device locale on
            first launch (UA region → UK; otherwise EN). Tap to flip; the change persists
            via the i18n provider. Animated pulse on the active letter for clarity. */}
        <Pressable
          onPress={() => setLang(lang === 'en' ? 'uk' : 'en')}
          style={s.langToggle}
          testID="welcome-lang-toggle"
          accessibilityRole="button"
          accessibilityLabel={lang === 'en' ? 'Switch to Ukrainian' : 'Switch to English'}
          hitSlop={10}
        >
          <Text style={s.langToggleActive} testID="welcome-lang-current">
            {lang === 'en' ? 'EN' : 'UK'}
          </Text>
          <View style={s.langToggleSep} />
          <Text style={s.langToggleAlt} testID="welcome-lang-alt">
            {lang === 'en' ? 'UK' : 'EN'}
          </Text>
        </Pressable>
      </View>
...
const s = StyleSheet.create({
  ...
  /* Compact language toggle — single pill, active letter + faded alternate.
     Tap anywhere on the pill to flip. Visually lighter and more interactive than
     two-pill switcher: small footprint, one tap target, no decision UI. */
  langToggle: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
