    ...
    <SafeAreaView style={s.flex} edges={['top']}>
      ...
      >
        ...
        <View style={s.headerRow}>
          ...
          {showPicker ? (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Text style={s.projectPillText}>{project.title}</Text>
              <Ionicons name="chevron-down" size={14} color={T.textSecondary} />
            </TouchableOpacity>
          ) : (
            <Text style={s.projectStatic}>{project.title}</Text>
          )}
        </View>
        ...
        {current_work.length === 0 ? (
          <Text style={s.empty}>
            <Text>Nothing active right now.</Text>
            {progress.queued > 0
              ? <Text>{` ${progress.queued} `}<Text>module(s) queued — picking up next.</Text></Text>
              : <Text> When the next phase starts, modules will appear here.</Text>}
          </Text>
        ) : (
...
const s = StyleSheet.create({
  ...
  /* Header */
  headerRow:    { flexDirection: 'column', alignItems: 'stretch', marginBottom: T.lg, gap: 8 },
  ...
  projectPillText: { color: T.textSecondary, fontSize: 13, fontWeight: '600', flexShrink: 1 },
  projectStatic:   { color: T.textMuted, fontSize: 13, fontWeight: '500' },
