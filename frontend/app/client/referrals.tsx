...
export default function ClientReferralsScreen() {
  ...
  return (
    ...
    >
      ...
      {/* BLOCK 1 — Header */}
      <View style={s.header}>
        <Text style={s.h1} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>
          Earn with EVA-X
        </Text>
        <Text style={s.h1Sub}>
          {currentRate === 0.07
            ? 'Get 7% from every project your referrals build.'
      ...
      <View style={s.card} testID="referrals-link-card">
        ...
        <View style={s.linkActionRow}>
          <TouchableOpacity
            ...
            disabled={!fullUrl}
          >
            <Ionicons name={copied ? 'checkmark' : 'copy-outline'} size={14} color={copied ? T.primaryInk : T.bg} />
            <Text style={[s.linkBtnText, copied && { color: T.primaryInk }]} numberOfLines={1}>
              {copied ? 'Copied' : 'Copy link'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity testID="referrals-share-btn" style={s.linkShareBtn} onPress={onShare} disabled={!fullUrl}>
            <Ionicons name="share-outline" size={14} color={T.primary} />
            <Text style={s.linkShareBtnText} numberOfLines={1}>Share</Text>
          </TouchableOpacity>
        </View>
        {link ? (
...
const s = StyleSheet.create({
  ...
  linkBtn: {
    ...
    backgroundColor: T.primary, borderRadius: T.radiusSm, paddingVertical: 11, paddingHorizontal: 8,
    minWidth: 0,
  },
  linkBtnText: { color: T.bg, fontWeight: '800', fontSize: T.small, flexShrink: 1 },
  linkShareBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4,
    backgroundColor: T.surface2, borderRadius: T.radiusSm, paddingVertical: 11, paddingHorizontal: 8,
    borderWidth: 1, borderColor: T.primary,
    minWidth: 0,
  },
  linkShareBtnText: { color: T.primary, fontWeight: '700', fontSize: T.small, flexShrink: 1 },
